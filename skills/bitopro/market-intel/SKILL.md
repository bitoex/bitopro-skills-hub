...
- **filtering:** Skip or flag pairs with `maintain: true`.
...
⚠️ 維護流程中（若有）: {pair 清單與基本規格}
```

**Grouping**: by quote currency (TWD / USDT / BTC).
**Maintenance**: mark rows with ⛔ and list at the bottom.
**Delisting**: if pair's `base` is not in the BitoPro coin mapping, exclude from main table and note separately under "⚠️ 維護流程中".
